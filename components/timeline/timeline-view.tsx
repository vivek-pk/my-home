...
import { Calendar, Clock, CheckCircle, AlertCircle, Plus, MessageSquare } from "lucide-react"
...
function getPhaseStatus(phase: ProjectPhase) {
  ...
  if (isAfter(now, end) && phase.status !== "completed") return "overdue"
...
function TimelineView({ project, canEdit = false, onUpdate }: TimelineViewProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {project.timeline.map((phase, index) => {
          ...
          const duration = differenceInDays(new Date(phase.endDate), new Date(phase.startDate))
          const isLast = index === project.timeline.length - 1

          return (
            <div key={phase._id || index} className="relative">
              {/* Timeline connector */}
              {!isLast && <div className="absolute left-6 top-16 w-0.5 h-8 bg-border" />}

              <Card className={cn("transition-all hover:shadow-md", status === "active" && "ring-2 ring-primary/20")}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      <div className={cn("p-2 rounded-full border", getStatusColor(status))}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="flex items-center space-x-2">
                      ...
                      <span>Start: {format(new Date(phase.startDate), "MMM d, yyyy")}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Calendar className="h-4 w-4 text-muted-foreground" />
                      <span>End: {format(new Date(phase.endDate), "MMM d, yyyy")}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        {phase.updates.length > 2 && (
                          <div className="text-sm text-muted-foreground">+{phase.updates.length - 2} more updates</div>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  {canEdit && (
                    <div className="flex gap-2 pt-2 border-t">
                      <Button variant="outline" size="sm" onClick={() => handleAddUpdate(phase)}>
                        <MessageSquare className="mr-2 h-4 w-4" />
                        Add Update
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => handleManageMaterials(phase)}>
