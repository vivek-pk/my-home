...
async function getProjectTeam() {
  ...
  // Get team members
  const teamMemberIds = [...(project.engineerIds || []), ...(project.managerIds || [])]
  const teamMembers = await Promise.all(teamMemberIds.map((id: string) => getUserById(id)))
    ...
    return (
      <HomeownerLayout>
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">You don't have any construction projects assigned yet.</p>
        </div>
      </HomeownerLayout>
    )
  }
    ...
    <HomeownerLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Get in touch with your project team members</p>
        </div>
        ...
        {managers.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your main points of contact for project oversight</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {managers.map((manager) => (
                  <div key={manager?._id} className="flex items-center justify-between p-4 border rounded-lg">
        ...
        {engineers.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Technical team members working on your project</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {engineers.map((engineer) => (
                  <div key={engineer?._id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex items-center space-x-4">
        ...
        <Card className="border-orange-200 bg-orange-50/50">
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-medium text-orange-800">24/7 Emergency Line</h3>
                <p className="text-sm text-orange-700">Available for urgent construction issues</p>
              </div>
              <Button
                variant="outline"
