...
function getProjectStatus(project: any) {
  ...
  if (project.status === "completed") return { status: "Completed", color: "bg-green-100 text-green-800" }
  if (project.status === "on-hold") return { status: "On Hold", color: "bg-yellow-100 text-yellow-800" }
  if (progress === 0) return { status: "Starting Soon", color: "bg-blue-100 text-blue-800" }
  if (progress < 100) return { status: "In Progress", color: "bg-orange-100 text-orange-800" }
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {projects.map((project) => {
                ...
                const activeTasks = project.timeline.filter(
                  (phase: any) => phase.status === "in-progress" || phase.status === "pending",
                ).length
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-muted-foreground text-center">
                  You haven't been assigned to any construction projects yet. Contact your administrator for project
                  assignments.
                </p>
              </CardContent>
            </Card>
